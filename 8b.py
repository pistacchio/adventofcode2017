...
instructions_raw = [t.strip() for t in INPUT.split('\n') if t.strip() != '']
instructions = []
for instruction in instructions_raw:
    reg, op, arg, _, ifreg, comp, ifarg = instruction.split(' ')
    instructions.append({
        'reg':   reg,
        'op':    op,
        'arg':   int(arg),
...
register_names = set(i['reg'] for i in instructions)
registers = {r: 0 for r in register_names}

highest = 0
for instruction in instructions:
    comp = OPERATORS[instruction['comp']]
    ifreg = registers[instruction['ifreg']]
    reg = instruction['reg']

    if comp(ifreg, instruction['ifarg']):
        op = OPS[instruction['op']]
        registers[reg] = op(registers[reg], instruction['arg'])
