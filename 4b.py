...
for passphrase in [pf.strip() for pf in INPUT.splitlines() if pf.strip() != '']:
    words = [''.join(sorted(w.strip())) for w in passphrase.split(' ') if w.strip() != '']
    words = sorted(words)
    groups = groupby(words)
    word_count = [len(list(g[1])) for g in groups]
    invalid = any(w for w in word_count if w > 1)
