...
towers_raw = [t.strip() for t in INPUT.split('\n') if t.strip() != '']
towers = {}

for tower_raw in towers_raw:
    splitted        = re.match(r'(.*?)\s\((\d+)\)(\s->(.*))?', tower_raw)
    name            = splitted.group(1)
    weight          = int(splitted.group(2))
    children = []
    if splitted.group(4):
        children = [c.strip() for c in splitted.group(4).strip().split(',')]
...
for _, tower in towers.iteritems():
    tower['children'] = [towers[c] for c in tower['children']]
    for tower_c in tower['children']:
        tower_c['parent'] = tower

root_key = [t for t in towers if not towers[t]['parent']][0]
root = towers[root_key]

def calc_weight (tower):
    for tower_c in tower['children']:
        calc_weight(tower_c)
    tower['computed_weight'] = tower['weight'] + sum([c['computed_weight'] for c in tower['children']])
...
def find_unbalanced(tower):
    tower['children'] = sorted(tower['children'], key=lambda c: c['computed_weight'])
    tower_groups = groupby(tower['children'], lambda c: c['computed_weight'])

    different_tower = None
    for tg in tower_groups:
        tgl = list(tg[1])
        if len(tgl) == 1:
            different_tower = tgl[0]
            break
    return different_tower

current_tower = root
while True:
    unbalanced_tower = find_unbalanced(current_tower)
    if not unbalanced_tower:
        break
    current_tower = unbalanced_tower

unbalanced_tower_weight          = current_tower['weight']
unbalanced_tower_computed_weight = current_tower['computed_weight']

balanced_towers_computed_weight = [c for c in current_tower['parent']['children'] if c['name'] != current_tower['name']][0]['computed_weight']
balance_diff = unbalanced_tower_computed_weight - balanced_towers_computed_weight

print unbalanced_tower_weight - balance_diff